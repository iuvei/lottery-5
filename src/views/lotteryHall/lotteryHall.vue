	...
	<div class="lottery-hall">
		...
		<router-link to="/myMessage">
			<div class="nocice">
				...
				<span style="color: #6c6c6c">{{notice?notice[0].title : ''}}</span>
			</div>
		</router-link>
...
<script>
	...
	import {setSscRebase} from '../../utils/auth'
	...
	export default {
		...
		methods: {
			...
			async getRebase() {
				...
				let res = await this.axios.get(`/v1/Lottery/GetRebate?type=ssc`)
				setSscRebase(res.data.data)
			},
			toAllLottery() {
